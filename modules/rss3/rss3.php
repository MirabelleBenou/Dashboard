<?php
	...
	echo '<style type="text/css">.rss3_item { height: '.$hauteur_items.'%; }</style>';
    ...
    echo '<img src="./modules/rss3/'.$logo.'" title="rss3" alt="rss3">';
	
	// Récupération des articles Rss
	$rss3 = simplexml_load_file($url);
	if(!$rss3){
		// Cas d'un fichier incorrect
        echo '<div class="rss3_item">';
        echo '<div class="rss3_titre">Fichier RSS incorrect</div>';
        echo '<div class="rss3_texte">Le fichier '.$url.' n est pas correct !</div>';
        echo '</div>';
	}
	else {
		// Cas où c'est OK !
		...
		foreach($rss3->channel->item as $item) {
			...
			$i++;
	        echo '<div class="rss3_item">';
	        echo '<div class="rss3_titre">';
	        echo '<a href="'.$item->link.'" title="'.$item->title.'">'.$item->title.'</a>';
	        echo '</div>';
	        echo '<div class="rss3_texte">'.strip_tags($item->description).'</div>';
	        echo '</div>';
		}
	}
