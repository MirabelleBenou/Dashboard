<?php
	...
	echo '<style type="text/css">.rss5_item { height: '.$hauteur_items.'%; }</style>';
    ...
    echo '<img src="./modules/rss5/'.$logo.'" title="rss5" alt="rss5">';
	
	// Récupération des articles Rss
	$rss5 = simplexml_load_file($url);
	if(!$rss5){
		// Cas d'un fichier incorrect
        echo '<div class="rss5_item">';
        echo '<div class="rss5_titre">Fichier RSS incorrect</div>';
        echo '<div class="rss5_texte">Le fichier '.$url.' n est pas correct !</div>';
        echo '</div>';
	}
	else {
		// Cas où c'est OK !
		$i = 0;
		foreach($rss5->channel->item as $item) {
			if($i == $nb_items_a_afficher) break;
			$i++;
	        echo '<div class="rss5_item">';
	        echo '<div class="rss5_titre">';
	        echo '<a href="'.$item->link.'" title="'.$item->title.'">'.$item->title.'</a>';
	        echo '</div>';
	        echo '<div class="rss5_texte">'.strip_tags($item->description).'</div>';
	        echo '</div>';
		}
	}
