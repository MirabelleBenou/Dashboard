<?php
	...
	echo '<style type="text/css">.rss4_item { height: '.$hauteur_items.'%; }</style>';
    ...
    echo '<img src="./modules/rss4/'.$logo.'" title="rss4" alt="rss4">';
	
	// Récupération des articles Rss
	$rss4 = simplexml_load_file($url);
	if(!$rss4){
		// Cas d'un fichier incorrect
        echo '<div class="rss4_item">';
        echo '<div class="rss4_titre">Fichier RSS incorrect</div>';
        echo '<div class="rss4_texte">Le fichier '.$url.' n est pas correct !</div>';
        echo '</div>';
	}
	else {
		// Cas où c'est OK !
		...
		foreach($rss4->channel->item as $item) {
			...
			$i++;
	        echo '<div class="rss4_item">';
	        echo '<div class="rss4_titre">';
	        echo '<a href="'.$item->link.'" title="'.$item->title.'">'.$item->title.'</a>';
	        echo '</div>';
	        echo '<div class="rss4_texte">'.strip_tags($item->description).'</div>';
	        echo '</div>';
		}
	}
